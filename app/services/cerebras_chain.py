...
async def intelligent_file_update(
    ...
    """
    try:
        print(f"[FILE UPDATE] Processing: {attachment.filename}")
        ...
        try:
            ...
            if not new_content or len(new_content) < 10:
                print(f"[FILE UPDATE] ❌ Generated content too short")
                return None
            ...
            print(f"[FILE UPDATE] ✅ {attachment.filename} updated to v{next_version}")
            print(f"[FILE UPDATE] Summary: {summary}")
            
            return new_att
            
        except json.JSONDecodeError as e:
            print(f"[FILE UPDATE] ❌ Failed to parse AI response: {e}")
            return None
            
    except Exception as e:
        print(f"[FILE UPDATE] ❌ Error updating {attachment.filename}: {e}")
        return None
# ==================== MAIN AI CHAIN ====================
...
async def ai_chain_stream(
    ...
    print(f"\n[AI CHAIN] Starting for conv {conv_id}")
    print(f"[AI CHAIN] Query: {user_query[:100]}...")
    ...
    try:
        ...
        if any(keyword in user_query.lower() for keyword in search_keywords):
            ...
            try:
                ...
                print(f"[AI CHAIN] Web search error: {e}")
