...
if not CEREBRAS_API_KEY: raise ValueError("CEREBRAS_API_KEY environment variable tidak ditemukan!")
...
async def get_ai_response(context: List[Dict[str, str]]) -> str:
    ...
    system_prompt = {
        ...
        1.  If the user writes in Indonesian, you MUST reply in a casual 'Jaksel' (South Jakarta) style. Mix Indonesian and English naturally (e.g., 'literally', 'which is', 'actually', 'bestie'). Be chill and friendly.
        2.  If the user writes in English, you MUST reply using modern, casual American slang. Use phrases like 'no cap', 'bet', 'spill the tea', 'that's dope', 'vibe check', 'lowkey', etc. Sound like a young, trendy American.
        3.  If the user writes in Javanese (Basa Jawa), you MUST reply using the specific, informal 'Suroboyoan' dialect. Use words like 'cok', 'jan***', 'arek-arek', 'sepurane', 'iyo', 'gak', 'lapo'. Be direct and friendly like a Surabayan.
...
def register_handlers(client: Client):
    ...
    async def id_command(c: Client, message: Message):
        ...
        if message.chat.type != ChatType.PRIVATE: text += f"💬 **ID chat ini tuh:** `{message.chat.id}`\n"
    ...
    async def add_user_command(c: Client, message: Message):
        ...
        except IndexError: await send_log_notification("**Perintah Gagal:** Formatnya `.add <session_string>` ya, beb.", is_error=True); return
        ...
        try:
            ...
            await new_client.start(); me = new_client.me
            ACTIVE_CLIENTS[me.id] = new_client; auto_reply_states[me.id] = True
            register_handlers(new_client); await join_log_group(new_client)
    ...
    async def gcast_command(c: Client, message: Message):
        ...
        except IndexError: await message.edit_text("Eh, formatnya salah cuy. *It should be* `.gcast <pesan>`"); return
    ...
    async def gucast_command(c: Client, message: Message):
        try: text_to_send = message.text.split(" ", 1)[1]
        except IndexError: await message.edit_text("Salah format, *babe*. Coba deh `.gucast <pesan>`"); return
        await message.delete(); count = 0
        async for dialog in c.get_dialogs():
...
async def main():
    ...
    if not initial_sessions: raise ValueError("Gak ada SESSION env, gimana mau login?")
    ...
    if not ACTIVE_CLIENTS:
        logging.critical("Sumpah, gak ada satu pun klien yang bisa connect. Bot mati."); return
    ...
    for client in ACTIVE_CLIENTS.values():
        ...
        startup_message = f"**Akun connect:** {me.first_name} (@{me.username})"
